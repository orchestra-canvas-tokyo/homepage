...
5周年記念の静的特設ページを `/special/5th-alps` に配置しています。

- トップページに特設ページへの導線を表示しています。
- 第17回定期演奏会ページにも導線を表示しています。
- 第1回・第17回定期演奏会の既存公演データとフライヤーを参照しています。
- 「5年間を数字で」の演奏会数・演奏曲数は、定期演奏会と室内楽演奏会を合わせて集計しています。
- OCTスロットの動画候補は `src/routes/special/5th-alps/data.ts` の `octSlotVideos` で管理しています。
- Emoji Reaction はローカル状態のみで、永続化・投稿・外部API連携はありません。

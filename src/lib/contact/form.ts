import { z } from 'zod';
import { maxTurnstileTokenLength } from './captcha';
...
const contactRequestSchema = z.object({
	...
	turnstileToken: z
		.string()
		.min(1, 'Turnstile の検証に失敗しました。')
		.max(maxTurnstileTokenLength, 'Turnstile の検証に失敗しました。')
});
...
export const validateContactRequest = (values: Record<string, unknown>) =>
	contactRequestSchema.safeParse({
		...
		categoryKey: toStringValue(values.categoryKey),
		body: toStringValue(values.body),
		turnstileToken: toStringValue(values['cf-turnstile-response'])
	});
